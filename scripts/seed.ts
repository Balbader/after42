  ...
  return [
    {
      ...
      interviewGuide: [
        ...
        {
          question: "If you had another day, what would you add or change?",
          expected_answer:
...
async function cleanup() {
  ...
  await db.delete(candidateSubmission).where(like(candidateSubmission.id, "seed-%"));
  await db.delete(challengeCounter).where(like(challengeCounter.challengeId, "seed-%"));
  await db.delete(challenge).where(like(challenge.id, "seed-%"));
  await db.delete(jobPost).where(like(jobPost.id, "seed-%"));

  if (seedUserIds.length > 0) {
    await db.delete(session).where(inArray(session.userId, seedUserIds));
    await db.delete(account).where(inArray(account.userId, seedUserIds));
    await db.delete(user).where(inArray(user.id, seedUserIds));
  }

  console.log(`  Removed ${seedUserIds.length} users and associated data.\n`);
}
...
async function createUsers(): Promise<Record<string, string>> {
  ...
  for (const u of SEED_USERS) {
    try {
      ...
      await db.update(user).set({ emailVerified: true }).where(eq(user.email, u.email));
