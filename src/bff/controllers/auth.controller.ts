...
export class AuthController {
  ...
  async signUp(formData: FormData, headers?: Headers) {
    ...
    if (!validation.success) {
      ...
      message(`AuthController: Sign up validation failed - ${firstError.message}`);
      return {
        success: false,
    ...
    const result = await authService.signUp(validation.data, "/dashboard", headers);
  ...
  async signIn(formData: FormData, headers?: Headers) {
    ...
    if (!validation.success) {
      const firstError = validation.error.issues[0];
      message(`AuthController: Sign in validation failed - ${firstError.message}`);
      return {
        success: false,
        error: firstError.message,
        fieldErrors: validation.error.flatten().fieldErrors,
    ...
    // Call service layer (pass headers so nextCookies() can set session cookie)
    const result = await authService.signIn(validation.data, "/dashboard", headers);
