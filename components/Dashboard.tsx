			...
			<div className="mb-8 flex justify-between items-start">
				<div>
					...
					<p className="text-base text-gray-400 leading-relaxed">
						...
						<span className="font-medium text-[#00ADB5]">
							{jobs.reduce(
								(sum, job) => sum + job.candidatesCount,
								0,
							)}{' '}
							42 students
						</span>{' '}
						proving their skills through AI-powered technical
						challenges.
					</p>
				</div>
				...
				<div className="flex items-center gap-4">
					...
					<div className="relative">
						<Button
							variant="outline"
							size="icon"
							className="relative h-11 w-11 border-[#1A1A1A] bg-[#0F0F0F] hover:bg-[#1A1A1A] hover:border-[#00ADB5]/30 text-gray-400 hover:text-[#00ADB5] transition-all duration-200"
						>
							<svg
								className="w-5 h-5"
								fill="none"
									...
									>
										...
										<Tooltip
											contentStyle={{
												...
												borderRadius: '8px',
												boxShadow:
													'0 4px 6px -1px rgb(0 173 181 / 0.2)',
												color: '#F3F4F6',
											}}
										/>
										<Bar
				...
				{filteredJobs.length === 0 ? (
					...
					<div className="grid gap-4">
						{filteredJobs.map((job) => (
							<Card
								key={job.id}
								className="group hover:shadow-2xl hover:shadow-[#00ADB5]/20 transition-all duration-300 bg-gradient-to-br from-[#0F0F0F] to-[#0A0A0A] border-[#1A1A1A] hover:border-[#00ADB5]/50"
							>
								<CardContent className="p-6">
									<div className="flex items-center justify-between">
										<div className="flex-1">
											<div className="flex items-center gap-3 mb-3">
												<h3 className="text-lg font-medium text-white group-hover:text-[#00ADB5] transition-colors duration-200">
													{job.title}
												</h3>
												<Badge
													className={`${getStatusColor(job.status)} font-medium border`}
												>
													{job.status}
												</Badge>
												{job.aiGenerated && (
